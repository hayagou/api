...
//    @Override
//    protected void configure(HttpSecurity http) throws Exception {
//        http.httpBasic().disable() // rest api 이므로 기본설정 사용 안함, 기본 설정은 비인증시 로그인폼 화면으로 리다이렉트 된다.
//                .csrf().disable() // rest api 이므로 csrf 보안이 필요없으므로 disable 처리
//                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS) // jwt token으로 인증하므로 세션은 필요없으므로 생성 안함
//                .and().authorizeRequests() // 다음 리퀘스트에 대한 사용권한 체크
//                .antMatchers("/v1/signin", "/v1/signup").permitAll() // 가입 및 인증 주소는 누구나 접근 가능
